...
import mimetypes
from app.schemas.schema_subtiter import SchemaUserSegmentVideoList, SchemaUserSegmentVideo, SchemaUserVideoCard, VideoAspectRatio
from typing import List
from app.db_polling.user_videos_polling import process_user_video
from app.service.subtiter.subscription_plan_guard import __is_user_allowed_to_generate_shorts
from app.subtitercmd.clippercmd.model.short_config_model import ShortConfigJson, SubtiterShortsConfig, SubtitleApplication, SubtitleConfigurationUI, SubtitleConfiguration, TrimApplication, TrimConfiguration, TrimConfigurationUI
from app.db.model_document import VideoProcessingApplication
from app.subtitercmd.filesystem_wrapper.filesystem_wrapper import make_file_remote
# Configure logger
...
router = APIRouter(prefix="/user-trimming", tags=["user_trimming"])




def _get_generated_trimming_videos_info(user_id: str, video_id: str) -> List[TrimConfigurationUI]:
    """
    ...
    if not video_path.exists():
        ...
        warehouse_dir = FilePath("app/subtiter_warehouse")
        video_path = warehouse_dir / user_id / video_id / "videos-cropped-stacked"
    ...
    info_files = list(video_path.glob('segment_*.trimmed.json'))
    logger.info("=== USER_VIDEOS DEBUG: subtitle files: %s ===", info_files)
    # create SchemaUserSegmentVideo object from the json content
    if info_files:
        trim_configurations = [TrimConfiguration.model_validate(json.load(open(f, 'r'))) for f in info_files]
        trim_configuration_uis = [ TrimConfigurationUI(
                trim_configurations=[trim_configuration],
                video_url=f"/user-trimming/serve/{user_id}/{video_id}/{index+1}",
                thumbnail_url=f"/user-shorts/thumbnail/{video_id}/{index+1}",
            ) for index, trim_configuration in enumerate(trim_configurations)]
        return trim_configuration_uis
    return []

@router.post("/generate-trimming/{video_id}")
async def generate_trimming(
    request: Request,
    video_id: str,
    trim_application: TrimApplication = Body(...),
    ...
    config_json = SubtiterShortsConfig()

    config_json.config_json.original_video_duration_in_seconds = user_video.video_duration
    config_json.config_json.applied_application = VideoProcessingApplication.APPLY_TRIM
    config_json.config_json.trim_application = trim_application
    user_video.config_json = config_json.model_dump(mode='json')
    user_video.applied_application = VideoProcessingApplication.APPLY_TRIM.value
    user_video.status = UserVideoStatus.PROCESSING.value
    db.commit()
    db.refresh(user_video)
    process_user_video(user_video, simulate=False)
    return {
        "message": f"Trimming generation started",
        "video_id": video_id,
    }
...
@router.get("/generated-trimming-info/{video_id}", response_model=List[TrimConfigurationUI])
async def get_generated_trimming_info(
    request: Request,
    ...
    user_id = request.state.user_id
    trim_configuration_uis = _get_generated_trimming_videos_info(user_id, video_id)
    return trim_configuration_uis


@router.get("/serve/{user_id}/{video_id}/{index}")
async def serve_trimming_video_file(
    request: Request,
    ...
    try:
        ...
        short_filename = f"segment_{index}.trimmed.mp4"

...
def _create_karaoke_effect(
    ...
    """
    Generates a single box for the line and a static text event for each word,
    but without any word-by-word karaoke animation.
    """
    ...
    # --- Create a single box for the entire line ---
    max_word_height = max(w['height'] for w in word_info) if word_info else 0
    box_height = max_word_height + (2 * padding_y)
    box_width = total_line_width
    ...
    box_event = pysubs2.SSAEvent(
        start=original_line.start, end=original_line.end,
        text=(
            f"{{\\an7\\pos({box_x:.1f},{box_y:.1f})\\p1"
            f"\\c{subtitle_box_background_color}\\1a&H{subtitle_box_transparency:02X}&"
            f"\\bord0}}{drawing_commands}{{\\p0}}"
        ),
        style=STYLE_DEFINITION["Name"], layer=0
    )
    box_events = [box_event]

    # --- Create text events with a single color ---
    text_events = []
    current_x = start_x

    for word_data in word_info:
        _, display_text, word_width, _ = word_data.values()

        word_box_width = word_width + (2 * padding_x)
        ...
        # Create a single text event for each word, visible for the whole line duration
        text_content = f"{{\\c{active_color}}}{base_text_tags}{display_text}"
        ...
        current_x += word_box_width + word_spacing

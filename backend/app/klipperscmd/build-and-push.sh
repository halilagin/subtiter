...
REGION=${1:-eu-west-1}
AWS_ACCOUNT_ID=${2:-$(kaws_klippers_cli sts get-caller-identity --query Account --output text)}
ECR_REPOSITORY="klipperscmd"
IMAGE_TAG=${3:-latest}

echo "=== Building and Pushing Klipperscmd Docker Image ==="
echo "Region: $REGION"
...
kaws_klippers_cli ecr get-login-password --region $REGION | docker login --username AWS --password-stdin ${AWS_ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com

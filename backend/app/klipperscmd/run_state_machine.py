...
This orchestrates parent-child Fargate task execution
...
PARENT TASK receives:
  ...
  - FARGATE_EXECUTION_ROLE: "parent"

CHILD TASKS receive:
  - TASK_INPUT: Same JSON string as parent (no modifications)
  - SEGMENT_NUMBER: The segment index (1, 2, 3, ..., segment_count)
  - FARGATE_EXECUTION_ROLE: "child"

The state machine:
  1. Runs parent task with FARGATE_EXECUTION_ROLE=parent
  2. Reads segment_count from S3 (shorts_config.json)
  3. Creates array [1..segment_count]
  4. Runs child tasks in parallel, each with its SEGMENT_NUMBER
"""
...
def main():
    print_info("=== Klippers Video Processing State Machine ===")
    ...
    try:
        ...
        for state_machine in response.get('stateMachines', []):
            if state_machine['name'] == 'klippers-video-processing':
                state_machine_arn = state_machine['stateMachineArn']
                break
        
        if not state_machine_arn:
            print_error("ERROR: State machine 'klippers-video-processing' not found")
        ...
        execution_name = f"vid-proc-{user_id_short}-{video_id_short}-{timestamp}"
        ...
        print(f"  aws logs tail /aws/stepfunctions/klippers-video-processing --follow")
        print()
        print("View Fargate task logs:")
        print(f"  aws logs tail /ecs/klippers --follow")
        print()

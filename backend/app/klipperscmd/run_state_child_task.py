...
- CLUSTER_NAME: ECS cluster name (default: klippers-cluster)
- TASK_DEFINITION_NAME: ECS task definition (default: klippers-video-processing)
"""
...
def main():
    print_info("=== Klippers Child Task Runner ===")
    print_info("Running a single child Fargate task directly")
    print()
    
    # Get required configuration from environment variables
    user_id = os.environ.get("USER_ID")
    video_id = os.environ.get("VIDEO_ID")
    segment_number = os.environ.get("SEGMENT_NUMBER")
    ...
    if not segment_number:
        print_error("ERROR: SEGMENT_NUMBER environment variable is required")
        sys.exit(1)
    ...
    s3_prefix = os.environ.get("S3_WAREHOUSE_PREFIX", "klippers_warehouse")
    mock_process = os.environ.get("MOCK_PROCESS", "false")
    fargate_token = os.environ.get("FARGATE_TOKEN", "fargate-token")
    webapp_api_host = os.environ.get("WEBAPP_API_HOST", "klippers.ai")
    webapp_api_port = int(os.environ.get("WEBAPP_API_PORT", "80"))
    webapp_api_host_fargate = os.environ.get("WEBAPP_API_HOST_FARGATE", "klippers.ai")
    webapp_api_port_fargate = int(os.environ.get("WEBAPP_API_PORT_FARGATE", "80"))
    webapp_api_url = os.environ.get("WEBAPP_API_URL", "https://klippers.ai")
    webapp_api_url_fargate = os.environ.get("WEBAPP_API_URL_FARGATE", "https://klippers.ai")
    chat_room_id = os.environ.get("CHAT_ROOM_ID", video_id)
    cluster_name = os.environ.get("CLUSTER_NAME", "klippers-cluster")
    task_definition_name = os.environ.get("TASK_DEFINITION_NAME", "klippers-video-processing")
    container_name = os.environ.get("CONTAINER_NAME", "klipperscmd")
    ...
    try:
        ...
        if not security_groups['SecurityGroups']:
            print_error("ERROR: Security group 'klippers-fargate-sg' not found")
            sys.exit(1)
        ...
        print_info("Starting child Fargate task...")
        ...
        print()
        print_success("✓ Child Fargate task started successfully!")
        print()
        ...
        print(f"  aws logs tail /ecs/klippers --follow --filter-pattern '{task_id}'")
        print()

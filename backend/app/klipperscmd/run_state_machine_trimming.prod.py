...
# flake8: noqa: E501
"""
Script to run the Step Functions state machine for trimming processing
This runs a single Fargate task for trimming
...
def main():
    print_info("=== Klippers Trimming Processing State Machine ===")
    ...
    try:
        ...
        for state_machine in response.get('stateMachines', []):
            if state_machine['name'] == 'klippers-trimming-processing':
                state_machine_arn = state_machine['stateMachineArn']
                break
        
        if not state_machine_arn:
            print_error("ERROR: State machine 'klippers-trimming-processing' not found")
        ...
        execution_name = f"trim-proc-{user_id_short}-{video_id_short}-{timestamp}"
        ...
        print(f"  aws logs tail /aws/stepfunctions/klippers-trimming-processing --follow")
        print()

...
**File:** `app/aws_app_stack/klippers_cognito.py`  
**Line:** 209-212
...
```html
<a href="https://klippers.ai/api/v1/auth/confirm-signup/{username}/{####}" class="verify-button">
...
```html
https://klippers.ai/api/v1/auth/confirm-signup/john@example.com/123456
```
...
|-------------------|--------|---------------|---------------|
| `{username}` | `Username` parameter in `sign_up()` | `john@example.com` | `klippers_cognito.py:211` |
...
From the docstring in `klippers_cognito.py` line 196:
...
```python
# auth.py line 148
klippers_cognito.register_user(
    email="john@example.com",  # ← Input
    ...
)

# klippers_cognito.py line 211
...
3. **You'll see:**
   - Link: `https://klippers.ai/api/v1/auth/confirm-signup/test@example.com/[6-digit-code]`
   - Code: `[6-digit-code]`
...
**A:** Yes! Just change line 211 in `klippers_cognito.py`:
...
- Link: `https://klippers.ai/api/v1/auth/confirm-signup/john%2Btest@example.com/123456`

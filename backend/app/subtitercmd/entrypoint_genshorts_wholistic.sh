...
echo "=== Subtitercmd Fargate Task Starting ==="
echo "Timestamp: $(date)"
...
stderr_path="$VIDEO_WAREHOUSE_ROOT_DIR/$USER_ID/$VIDEO_ID/subtiter.stderr"
stdout_path="$VIDEO_WAREHOUSE_ROOT_DIR/$USER_ID/$VIDEO_ID/subtiter.stdout"
...
set +e
timeout 50m python -m run_app_genshorts > "$stdout_path" 2> "$stderr_path"
exit_code=$?
set -e

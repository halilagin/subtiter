...
echo "=== Triggering Subtiter Video Processing ==="
echo "User ID: $USER_ID"
echo "Video ID: $VIDEO_ID"
echo "Region: $REGION"

# Get the state machine ARN
STATE_MACHINE_ARN=$(kaws_subtiter_cli stepfunctions list-state-machines --region $REGION --query "stateMachines[?name=='$STATE_MACHINE_NAME'].stateMachineArn" --output text)
...
# Start execution
EXECUTION_ARN=$(kaws_subtiter_cli stepfunctions start-execution \
    --region $REGION \
...
echo "kaws_subtiter_cli stepfunctions describe-execution --region $REGION --execution-arn $EXECUTION_ARN"
echo ""
echo "To view logs:"
echo "kaws_subtiter_cli logs tail /ecs/subtiter-video-processing --region $REGION --follow"

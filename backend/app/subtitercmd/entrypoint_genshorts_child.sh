...
echo "=== Subtitercmd Fargate Child Task Starting ==="
echo "Timestamp: $(date)"
...
export VIDEO_WAREHOUSE_ROOT_DIR=$(echo "$TASK_INPUT" | jq -r '.video_warehouse_root_dir // "/app/subtiter_warehouse"')
...
export S3_WAREHOUSE_PREFIX=$(echo "$TASK_INPUT" | jq -r '.S3_WAREHOUSE_PREFIX // "subtiter_warehouse"')
...
aws s3 cp --quiet $stderr_path "s3://$VIDEO_WAREHOUSE_S3_BUCKET_NAME/$S3_WAREHOUSE_PREFIX/$USER_ID/$VIDEO_ID/subtiter.$SEGMENT_NUMBER.stderr" || echo "Warning: Failed to upload results to S3"
aws s3 cp --quiet $stdout_path "s3://$VIDEO_WAREHOUSE_S3_BUCKET_NAME/$S3_WAREHOUSE_PREFIX/$USER_ID/$VIDEO_ID/subtiter.$SEGMENT_NUMBER.stdout" || echo "Warning: Failed to upload results to S3"
...
if [ $exit_code -eq 124 ]; then
    ...
    # The script would have already printed errors to stderr, which are in subtiter.stderr
    exit $exit_code
fi
...
WEBAPP_API_HOST_FARGATE=$(echo "$TASK_INPUT" | jq -r '.WEBAPP_API_HOST_FARGATE // "subtiter.ai"')
...
echo "=== Subtitercmd Fargate Task Completed ==="

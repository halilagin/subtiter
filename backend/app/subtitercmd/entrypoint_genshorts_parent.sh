...
echo "=== Subtitercmd Fargate Parent Task Starting ==="
echo "Timestamp: $(date)"
...
stderr_path="$VIDEO_WAREHOUSE_ROOT_DIR/$USER_ID/$VIDEO_ID/subtiter.parent.stderr"
stdout_path="$VIDEO_WAREHOUSE_ROOT_DIR/$USER_ID/$VIDEO_ID/subtiter.parent.stdout"
...
set +e
# timeout 30m python -m run_on_uploaded > "$stdout_path" 2> "$stderr_path"
timeout 30m python -m run_app_genshorts
exit_code=$?
set -e

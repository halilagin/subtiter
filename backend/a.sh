ffmpeg -i  ~/Downloads/klippers_demo_3x.mp4 -filter_complex \
   ...
   [0:v]trim=start=44,setpts=PTS-STARTPTS[v2]; \
    [0:a]atrim=start=0:end=4,asetpts=PTS-STARTPTS[a1]; \
     [0:a]atrim=start=44,asetpts=PTS-STARTPTS[a2]; \
      ...
      -map "[outv]" -map "[outa]" -c:v libx264 -preset fast -crf 23 -c:a aac  ~/Downloads/klippers_demo_3x_removed_4-44.mp4

...
import { roundedBoxColorOptions } from '@/constants/SubtitleColors';
...
interface RoundedBoxPreviewProps {
  colorOption: typeof roundedBoxColorOptions[0];
...
export const RoundedBoxPreviewCard: React.FC<RoundedBoxPreviewProps> = ({ 
  colorOption, 
  ...
  useEffect(() => {
    ...
    const handleSelectionChange = (event: CustomEvent) => {
      ...
      const cardId = `rounded_box_${colorOption.name}`;
  ...
  const handleToggleSelection = (e: React.MouseEvent) => {
    ...
    const cardId = `rounded_box_${colorOption.name}`;
    const cardName = `Rounded Box - ${colorOption.name}`;
    
    console.log('[RoundedBoxPreviewCard] Toggle selection:', { cardId, cardName, isCardSelected });
  ...
  const videoSrc = getSubtitleVideoBackground('rounded_box');
  ...
  return (
    <Box 
      sx={{ 
        ...
        overflow: 'hidden',
        bgcolor: '#000',
        border: isSelected ? `3px solid ${colorOption.ui.activeBoxColor}` : '3px solid transparent',
        cursor: 'pointer',
      ...
      onClick={() => onSelect(colorOption.ui.activeBoxColor)}
    >
      {/* Video Background */}
      <video
        src={videoSrc}
      ...
      >
        {words.map((word, index) => {
          ...
          if (!hasBeenShown && index > activeWordIndex) return null;
          ...
          return (
            <span
              ...
              style={{
                ...
                backgroundColor: isActive ? colorOption.ui.activeBoxColor : 'transparent',
                padding: isActive ? '1px 3px' : '0',
                border: isActive ? `1px solid ${colorOption.ui.activeBoxColor}` : 'none',
                borderRadius: '4px',
                transition: 'all 0.2s ease',
...
export const SubtitlingStyleRoundedBox: React.FC<SubtitlingStyleProps> = ({ capitalizationStyle, id }) => {
  ...
  const [currentPosition, setCurrentPosition] = useState<'top' | 'center' | 'bottom'>('center');
    ...
    useEffect(() => {
      ...
      let selectedColorOption = roundedBoxColorOptions.find((color) => color.ui.activeBoxColor === selectedColor);
      if (!selectedColorOption) {
        selectedColorOption = roundedBoxColorOptions[0];
      }

      const event = new CustomEvent(EventType.SUBTITLING_SELECT_SUBTITLE_STYLE, {
        detail: {
          subtitleStyle: 'rounded_box',
      ...
      console.log('[SubtitlingStyleRoundedBox] Dispatching subtitle-details-update:', detailsEvent.detail);
    ...
    <div style={{ 
          ...
          return (
            <span
              ...
              style={{
                ...
                backgroundColor: isActive ? (selectedColor ? selectedColor : roundedBoxColorOptions[0].ui.activeBoxColor) : 'transparent',
                padding: isActive ? '2px 4px' : '0',
                border: isActive ? `2px solid ${selectedColor ? selectedColor : roundedBoxColorOptions[0].ui.activeBoxColor}` : 'none',
                borderRadius: '4px',
                transition: 'all 0.2s ease',
              }}
            >
      ...
      {isSelected && (
        <button
          ...
          style={{
            ...
            left: '0',
            right: '0',
            backgroundColor: selectedColor ? selectedColor : roundedBoxColorOptions[0].ui.activeBoxColor,
            color: '#ffffff',
            border: 'none',
      ...
      {showColorDialog && ReactDOM.createPortal(
        <Box
          ...
          onClick={(e) => e.stopPropagation()} // Stop propagation to prevent re-triggering parent onClick
        >
          <Box
            sx={{
              bgcolor: '#1f2937',
          ...
          >
            ...
            <Box sx={{ 
              ...
              <Box sx={{ 
                ...
                {roundedBoxColorOptions.map((color) => (
                  <RoundedBoxPreviewCard
                    ...
                    onSelect={(colorValue) => {
                      ...
                      // Don't close dialog on selection to allow exploring other colors
                      // setShowColorDialog(false); 
                    }}
                    showSelectButton={true}
                    staysInSelectedBucket={false}
